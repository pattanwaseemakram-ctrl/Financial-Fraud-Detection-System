...
OUTPUT_DIR = "Dataset/smote enn"

X_OUTPUT_FILE = os.path.join(
    OUTPUT_DIR,
    "X_train_smote_enn.csv"
)

Y_OUTPUT_FILE = os.path.join(
    OUTPUT_DIR,
    "y_train_smote_enn.csv"
)
...
def save_data(X_resampled, y_resampled):
    """
    Save the SMOTEENN training data.
    """
...
def main():
    ...
    try:
        ...
        print(
            "\nTraining features shape after SMOTEENN:",
            X_resampled.shape
        )

        print(
            "Training target shape after SMOTEENN:",
            y_resampled.shape
        )
